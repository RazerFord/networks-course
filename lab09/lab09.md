...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - `IP`-адрес моего хоста `192.168.0.58`
   - `IP`-адрес хоста назначения `104.16.227.102`

   ![ping](./images/ping-1.png)
   ![wireshark](./images/wireshark-1.png)

2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - `ICMP` представляет протокол сетевого уровня, который используется для передачи информации об ошибках сети между устройствами, а не процессами, поэтому передавать порты нет необходимости
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - В `ICMP`-пакете так же имеются поля: `Checksum`, метаданные в зависимости от типа. Для `ICMP`-пакета типа `8` это `Identifier`, `Sequence Number`, `Data`
   - На `Checksum` приходится 2 байта, `Identifier` - 2 байта, `Sequence Number` - 2 байта

   ![content of icmp](./images/content-of-icmp.png)
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Аналогично запросу в `ICMP`-пакете имеются поля: `Checksum`, `Identifier`, `Sequence Number`, `Data`
   - На `Checksum` приходится 2 байта, `Identifier` - 2 байта, `Sequence Number` - 2 байта

   ![content of icmp response](./images/content-of-icmp-resp.png)
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Отличается типом, тип `ICMP` пакета, отправленного командой `traceroute` равен `11`. Так же можно заметить, что в `ICMP` пакет вложен `IP`-пакет
 
   ![traceroute](./images/traceroute.png)
   ![traceroute-wireshark](./images/traceroute-wireshark.png)
   ![traceroute-wireshark-request](./images/traceroute-wireshark-request.png)
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - В дополнительном поле содержится `IP`-пакет, который серверу назначения не удалось обработать. Внутри этого пакета содержится заголовок `IP` и `UDP`-пакет без полезной нагрузки
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - У последних трех сообщений отличается код `ICMP`-пакета, он равен `3`, в отличие от предыдущих, равных `0`
   - Порт назначения недоступен, это связано с тем, что мой порт на прием сообщений закрылся прежде чем сообщение пришло

   ![last-three-messages](./images/last-three-messages.png)
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Среднее время запросов через канал между маршрутизаторами под номером `14` и `15` больше, чем запросы через другие каналы
   - Маршрутизатор под номером `14` находятся в США, Калифорния, маршрутизатор под номером `15` находится в США, Канзас

   ![traceroute](./images/traceroute-2.png)
## Программирование.
