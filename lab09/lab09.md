...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   ![traceroute-wireshark](./images/traceroute-wireshark-2.png)
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - У последних трех сообщений отличается код `ICMP`-пакета, он равен `3`, в отличие от предыдущий, равных `0`
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - `1`-ый запрос через канал под номером `6`, проходил дольше, чем запросы через другие каналы
   - Маршрутизатор под номером `4` находится в Санкт-Петербург, маршрутизатор под номером `7` находится в Москве. Информация о маршрутизаторе номер `5` отсутствует
