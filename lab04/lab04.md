...
3. Проанализируйте ответное сообщение DNS. Имена каких DNS-серверов университета в
   ...
   - Содержатся следующие имена: `ns2.pu.ru`, `ns.pu.ru`, `ns7.spbu.ru`
   - Адреса не содержатся

      ![answer](./img/answer-4.png)
...
1. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию? 
   ...
   - Запрос отправлен на `ip` `195.70.196.210`

      ![query](./img/query-5.png)

   - `ip` указанный выше не совпадает с адресом локального `DNS` сервера. Принадлежит хосту `ns2.pu.ru`

      ![non-local](./img/nslookup-5.png)
2. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?
   - Запрашивается запись типа `A`
   - Ответы в запросе не содержатся

      ![query](./img/query-5.png)

3. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - В ответное сообщение входит два ответа
   - В первом ответе содержится псевдоним `spbu.ru`. Во втором ответе содержится `ip` `81.89.183.226`

      ![answer](./img/answer-5.png)

### Е. Сервисы whois (2 балла)
1. Что такое база данных whois?
   - Глобальная база данных доменов и `ip`-адресов, хранящая информацию о владельцах и другую информацию
...
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - У `tsu.ru` - `ns1.tsu.ru` и `ns2.tsu.ru`. У `spbu.ru` - `ns2.pu.ru`, `ns7.spbu.ru`  и `ns.pu.ru`

      ![tsu](./img/tsu-6.png)
      ![spbu](./img/spbu-6.png)

   - Использовал сервис `www.reg.ru/whois`

      ![reg-ru](./img/reg-ru-6.png)
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем конкретным
   ...
   найденным в предыдущей части.
      ...
      ![dns-1](./img/dns-1-6.png)
      ![dns-2](./img/dns-2-6.png)
      ![dns-3](./img/dns-3-6.png)
